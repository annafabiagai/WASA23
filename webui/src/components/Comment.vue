...
export default {	
	...
	props:['modalID','commentsList','isItMe','IDphoto'],

	methods: {
		async commentPhoto() {
            try {
                // POST /photos/{pid}/comments/
                let response = await this.$axios.post(`/photos/${this.IDphoto}/comments/`, this.commentText, {headers: {'Authorization': `${localStorage.getItem('token')}`, 'Content-Type': 'text/plain'}});
        ...
        async uncommentPhoto(IDcomment) {
            try {
                // DELETE /photos/{pid}/comments/{cid}
                await this.$axios.delete(`/photos/${this.IDphoto}/comments/${IDcomment}`, {headers: {'Authorization': `${localStorage.getItem('token')}`}});
                this.$emit('removeComment', IDcomment); // signal to parent
            } catch(error) {
                const status = error.response.status;
                const reason = error.response.data;
                this.errormsg = `Status ${status}: ${reason}`;
    ...
    <div class="modal fade my-modal-disp-none" :id="modalID" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div v-for="comment in commentsList" :key="comment.IDcomment">
                    <div class="modal-body">
                        <div class="container-fluid">
                            ...
                            <div class="row">
                                ...
                                <div class="col-2">
                                    <button v-if="token == comment.IDuser || isItMe" class="btn my-btn-comm" @click="uncommentPhoto(comment.IDcomment)">
